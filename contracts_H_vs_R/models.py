...
class Group(BaseGroup):
    ...
    def return_from_effort(self, effort):
        return Constants.reserve  + (effort + self.epsilon) * Constants.s
...
class Player(BasePlayer):
    ...
    task_1 = models.StringField(
        label='Задание 1. Во одном из раундов Исполнитель выбрал уровень усилий 2.5  на шаге 2. '
              'На шаге 3 компьютер посчитал значение Случайного фактора, оно составило  2. '
              'Какова будет Выручка Заказчика?')
    task_2 = models.StringField(
        label='Задание 2. Во втором этапе  Заказчик на шаге 1 в первом раунде предложил фиксированную часть вознаграждения '
              ' в размере 9, и переменную часть в размере 2.  Исполнитель предпринял уровень усилий 4 и '
              'компьютер на шаге 3 выбрал значение случайного фактора, оно составло - 1 (минус один). Сколько токентов получит Заказчик за раунд?')

    task_4 = models.StringField(
        label='Сколько получит Исполнитель за один раунд? ')

    task_5 = models.StringField(
        label='Задание 3. Исполнитель  на втором этапе в течение 20 раундов выигрывал в каждом раунде по 12 токенов, на первом этапе '
              '17 токена. Сколько рублей в конце эксеримента получит участник в роле исполнителя? '
              )

    task_6 = models.IntegerField(
        label=' Задание 4. Выберите правильный ответ на вопрос:  Исполнитель в одном из раундов второго этапа, выбрал уровень усилий '
              ' в размере 6. Какова вероятность что результат усилий с учетом случайного фактора выйдет из интервала от 3 '
              'до 9 ?',
        choices=[   [1 , '50%'],
                    [2 , '100%'],
                    [3 , 'прибизительно равна 95%'],
                    [4 , 'прибизительно равна 5% '],
                    [5 , 'прибизительно равна 1%'],
                    ]
        )


    social_media_time_spend = models.StringField(
        label= 'Укажите приблизительное количество стран, которое вы посетили?',
        choices=[['0-4', '0-4'],
                 ['5-9', '5-9'],
    ...
    numb_of_last_books = models.StringField(
        label='Укажите приблизительное количество книг, прочитанных вами за последние три месяца',
        choices=[['0-2', '0-2'],
                 ['3-5', '3-5'],
                 ['6 и более', '6 и более'],
                 ]
